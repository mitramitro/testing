    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request)
    {
        ...
        if($auth->hasIdentity()) {
            ...
            if(!$acl->isAllowed($user->role, $request->getControllerName() . '::' . $request->getActionName())) {
                ...
                //return Zend_Controller_Action_HelperBroker::getStaticHelper('redirector')->setGotoUrl('zendauth/home');
            }
        } else {
            if(!$acl->isAllowed($this->_defaultRole, $request->getControllerName() . '::' . $request->getActionName())) {
                $mysession->destination_url = $request->getPathInfo();
 
                return Zend_Controller_Action_HelperBroker::getStaticHelper('redirector')->setGotoUrl('zendauth/index');
            }
        }
    }
}
